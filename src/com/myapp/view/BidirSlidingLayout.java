...
public class BidirSlidingLayout extends RelativeLayout implements
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		Log.i(TAG,
				"BidirSlidingLayout onInterceptTouchEvent  " + action
						+ ev.getX() + "|" + ev.getY() + "|" + screenWidth);
		switch (action) {
		case MotionEvent.ACTION_DOWN:
			...
			if (!isRightMenuVisible
					&& !isLeftMenuVisible
					&& (ev.getX() < screenWidth * 0.15 || ev.getX() > screenWidth * 0.85)
					&& ev.getY() > screenHeight * 0.3){
		...
		case MotionEvent.ACTION_MOVE:
			...
			// mIsBeingDragged = true;
			if (mIsBeingDragged||isRightMenuVisible||isLeftMenuVisible) {
